...
df = pd.read_csv('home_rent.csv')

x = df.drop(columns=['locality', 'rent'], axis=1) 
y = df['rent']
...
st.set_page_config(page_title= "Rendal Property",
                   layout= "wide",
...
with st.sidebar:
    ...
    option = option_menu(menu_title='Menu', options=['Home', 'Migrating to SQL', 'Prediction', 'Data Analysis', 'Exit'],
                         icons=['house', 'database-fill', 'slash-square', 'bar-chart-line', 'sign-turn-right-fill'],
                         styles={'nav-link': {'--hover-color': '#20CA0C'}, 'nav-link-selected': {'--hover-color': '#20CA0C'}})

if option == 'Home':

    st.markdown(f'<h1 style="text-align: center; color: red;">Welcome to Rental Property Price Prediction</h1>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        col1.markdown("# ")
        col1.markdown("# ")
        col1.markdown("## :violet[*Overview*] : Build regression model to predict house rental price")
        col1.markdown("# ")
        col1.markdown("## :violet[*Domain*] : Real Estate & Property Management")
        col1.markdown("# ")
    ...
    with col2:
        col2.markdown("# ")
        col2.markdown("# ")
        col2.image("/Users/arul/Downloads/rent1.jpeg")
        col2.markdown("# ")
        col2.image("/Users/arul/Downloads/rent3.jpg")
...
if option == 'Migrating to SQL':
    ...
    with col2:
      ...
      def main():
        st.title(':red[SQL Table Operations]')
        st.markdown("# ")
        st.markdown("# ")
        ...
        st.markdown("### Click the Create Table button to create a Table in MySQL Database")
        create_table_button = st.button('Create Table')
        if create_table_button:
            sql_instance.create_table()
            st.success('Table created successfully!')

        st.markdown("# ")
        st.markdown("# ")
        
        st.markdown("### Click the Migrate Data button to migrate a data to MySQL Database")
        migrate_data_button = st.button('Migrate Data')
        if migrate_data_button:
            st.write(df)
            sql_instance.data_migration()
            st.success('Successfully Data Migrated to MySQL Database!')
            st.balloons()

        st.markdown("# ")
        st.markdown("# ")

        st.markdown("### Click the Drop Table button to drop the Table in MySQL Database")    
        drop_table_button = st.button('Drop Table')
        if drop_table_button:
            sql_instance.drop_table()
    ...
    def predict_rent():
        ...
        with st.form('prediction'):
            ...
            with col1:
                ...
                longitude = st.number_input(label='Longitude', min_value=77.50, max_value=80.27, value=77.50)
